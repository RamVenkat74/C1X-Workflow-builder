...
class C1XWorkflowBuilder extends HTMLElement {
    ...
    setupListeners() {
        ...
        canvas?.addEventListener('mousedown', (e) => {
            const target = e.target.closest('g.node-group');
            if (target) {
        ...
        canvas?.addEventListener('drop', (e) => {
            ...
            this.openConfigModal(e.dataTransfer.getData('type'), e.clientX - rect.left, e.clientY - rect.top);
        });
    ...
    openConfigModal(type, x, y) {
      ...
      <div class="modal-content">
        ...
        modal.querySelector('#add')?.addEventListener('click', () => {
            const text = modal.querySelector('#val').value || type;
            ...
            if (this.lastSplitId && (type === 'action' || type === 'end'))
                parentId = this.lastSplitId;
            ...
            if (parentId)
                this.engine.addEdge(parentId, id);
            this.renderNodeOnCanvas(id, type, x, y, text);
            this.updateEdges();
            modal.remove();
        });
    }
    renderNodeOnCanvas(id, type, x, y, text) {
        const canvas = this.shadowRoot?.querySelector('#canvas');
        ...
        if (type === 'wait' || type === 'end') {
            ...
            rect.setAttribute("fill", this.getColor(type));
    ...
    getColor(type) {
        const palette = { audience: '#eff6ff', filter: '#fff7ed', split: '#f0fdf4', action: '#fefce8' };
    ...
    updateEdges() {
        ...
        this.nodes.forEach(n => {
            if (n.parentId) {
                ...
                if (p)
                    this.drawManhattanEdge(p, n);
            }
        });
    }
    drawManhattanEdge(n1, n2) {
        const canvas = this.shadowRoot?.querySelector('#canvas');
